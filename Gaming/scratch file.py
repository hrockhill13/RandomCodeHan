...
def generate_unit(max_points):
    ...
    def add_unit(name, cost, unit_type, limit, unit_tags):
        ...
        if army_points + cost <= max_points:
            ...
            if max_points == 500:
                if monster in unit_tags or unique in unit_tags:
            ...
            if unit_counts[unit_type].get(name, 0) < limit:
                ...
                # Track points for astartes and daemon units
                ...
                return True
            print(f"Adding unit: {name}, Cost: {cost}, Tags: {unit_tags}")
    ...
    while army_points < max_points:
        ...
        while units_attempted < 10 and not unit_added:  # Try up to 10 random selections
            ...
            # After adding a unit, check if it exceeds the daemon points limit (close to 50%)
            if unit_added:
                if daemon_points + cost > ((max_points * 0.5)-375) and daemon in unit_tag:
                    # Undo the addition if it exceeds the daemon points limit
                    army_points -= cost
                    if daemon in unit_tag:
                        daemon_points -= cost
                    # Only remove from the correct unit list, 'Other Datasheets' if added there
                    if (name, cost) in csm_units['Other Datasheets']:
                        csm_units['Other Datasheets'].remove((name, cost))  # Remove the unit from the list if added
...
# Iterate through the units to display the list
